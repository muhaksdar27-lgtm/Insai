...
export function detectStrategy5Confluence(context: RuleEvaluationContext, pyData: any = {}): StrategyExecutionResult {
  ...
  const direction: SignalDirection | null = inferDirection({ explicit: s5.direction, bullish: (chochBull || sweepBull || bosBull || engulfBull || doubleBottom), bearish: (chochBear || sweepBear || bosBear || engulfBear || doubleTop), h1Trend });
  ...
  const setupSnapshot = {
    ...
    tp2: tp2Val || '--',
    tp2Price: tp2Val || '--',
    rr: s5.rr || candidateRules['rule_risk_reward']?.evidence?.rr || '--',
