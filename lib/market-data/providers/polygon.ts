...
export class PolygonProvider implements PriceProvider {
  ...
  private formatSymbol(symbol: string): string {
    ...
    if (norm === 'DXY') return 'I:DXY';
    if (norm === 'US10Y') return 'I:US10Y';
    return `C:${norm}`; // fallback to crypto
  }

  async getLatestPrice(symbol: string): Promise<MarketSnapshot> {
    if (symbol === 'DXY' || symbol === 'US10Y') {
        throw new Error(`Polygon free tier does not support indices like ${symbol}. Fast failing to next provider.`);
    }
    if (!this.currentApiKey) {
      throw new Error('Polygon API key is not configured');
    }
    const ticker = this.formatSymbol(symbol);
    
    try {
      logger.info(`Polygon REST: Fetching live price (input: ${symbol}, mapped: ${ticker})`);
