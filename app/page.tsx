...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-3 pb-12 relative min-h-screen">
      ...
      <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-lg p-2.5 flex flex-wrap items-center justify-between gap-2 shadow-md backdrop-blur-md">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded bg-blue-500/10 border border-blue-500/20 text-blue-400">
            <Terminal className="w-4 h-4" />
          </div>
          <div>
            <h2 className="text-xs font-mono font-bold text-zinc-100 uppercase tracking-widest flex items-center gap-2">
              COMMAND CENTER — SINGLE REALTIME SNAPSHOT
              <span className={`w-2 h-2 rounded-full ${ping ? 'bg-emerald-400 scale-125' : 'bg-emerald-500'} transition-all`} />
            </h2>
            <p className="text-[9px] font-mono text-zinc-400 flex items-center gap-1">
              Synchronized Snapshot Timestamp: <ClientDate date={masterTimestamp} />
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {error ? (
            <span className="text-[8px] font-mono font-bold text-rose-400 bg-rose-500/10 border border-rose-500/30 px-2 py-1 rounded flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> API DEGRADED
            </span>
          ) : (
            <span className="text-[8px] font-mono font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-2 py-1 rounded flex items-center gap-1">
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border border-zinc-700/80 text-[9px] font-mono font-bold tracking-wider uppercase transition-all disabled:opacity-50"
          >
      ...
      {snapshot && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            ...
            {/* Right Column - 1 Col (System Health, Economic News) */}
            <div className="space-y-3">
              <SystemHealthPanel
                system={snapshot.system}
              />
