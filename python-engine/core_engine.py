...
class CoreEngine:
    ...
    def _generate_cache_key(self, candles: List[Any]):
        if not candles:
            return None
        c = candles[-1]
        t = getattr(c, 'timestamp', c.get('timestamp', '')) if isinstance(c, dict) else getattr(c, 'timestamp', '')
        cl = getattr(c, 'close', c.get('close', 0)) if isinstance(c, dict) else getattr(c, 'close', 0)
        return hash((len(candles), t, cl))
    ...
    def analyze(self, candles: List[Any], provided_cache_key: str = None) -> Dict[str, Any]:
        ...
        # Fast Extraction
        try:
            # Ultra-fast path using generator to tuple list
        ...
        prev_swing_high = float(H[sh_indices[-2]]) if len(sh_indices) > 1 else 0.0
        
        last_swing_low = float(L[sl_indices[-1]]) if len(sl_indices) > 0 else 0.0
        prev_swing_low = float(L[sl_indices[-2]]) if len(sl_indices) > 1 else 0.0
        ...
        # Improved Double Top / Bottom
        double_top = False
        if len(sh_indices) >= 2:
            if abs(last_swing_high - prev_swing_high) < avg_body and is_red[-1] and (last_swing_high > 0):
                double_top = True
                
        double_bottom = False
        if len(sl_indices) >= 2:
            if abs(last_swing_low - prev_swing_low) < avg_body and is_green[-1] and (last_swing_low > 0):
                double_bottom = True

        # Improved Order Block Detection
        curr_ob_bull, curr_ob_bear = False, False
        if bos_bull or choch_bull or liq_sweep_bull:
            for j in range(N-2, max(-1, N-15), -1):
                if is_red[j] and body_size[j] > avg_body * 0.5:
                    if L[j] <= c_last <= H[j]:
                        curr_ob_bull = True
                    break
                    
        if bos_bear or choch_bear or liq_sweep_bear:
            for j in range(N-2, max(-1, N-15), -1):
