...
class SMCStrategy(BaseStrategy):
    ...
    def calculate_confidence(self, direction: str, analysis: Dict[str, Any], z_score: float) -> Tuple[int, List[str]]:
        ...
        if direction == 'LONG':
            ...
            if not analysis.get('ob_bull') and not analysis.get('fvg_bull_active') and not analysis.get('liq_sweep_bull') and not analysis.get('snd_bull'):
                score -= 20
                reasons.append("Missing primary SMC confluences (OB/FVG/Sweep/SnD)")
            ...
            # Filter low quality setups:
            if not analysis.get('ob_bear') and not analysis.get('fvg_bear_active') and not analysis.get('liq_sweep_bear') and not analysis.get('snd_bear'):
                score -= 20
                reasons.append("Missing primary SMC confluences (OB/FVG/Sweep/SnD)")
