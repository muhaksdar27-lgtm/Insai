    ...
    def calculate_confidence(self, direction: str, analysis: Dict[str, Any], z_score: float) -> Tuple[int, List[str]]:
        ...
        if direction == 'LONG':
            ...
            if analysis.get('snd_bull'):
                score += 30
                reasons.append("Strong Demand Base + Impulsive Break")
            
            if analysis.get('bullish_engulfing'):
                score += 20
                reasons.append("Demand Zone Rejection (Bullish Engulfing)")
            ...
            if analysis.get('snd_bear'):
                score += 30
                reasons.append("Strong Supply Base + Impulsive Break")
                
            if analysis.get('bearish_engulfing'):
                score += 20
                reasons.append("Supply Zone Rejection (Bearish Engulfing)")
